...
func (c *vlessSubTestClient) Status() VlessSubTestStatus {
	client := &http.Client{Timeout: 5 * time.Second}
	resp, err := client.Get(c.baseURL + "/test")
	if err != nil {
		return VlessSubTestStatus{Available: false, DaemonURL: c.baseURL, Error: err.Error()}
	}
	resp.Body.Close()
	return VlessSubTestStatus{Available: true, DaemonURL: c.baseURL}
}
...
func (c *vlessSubTestClient) TestSingle(vless string, timeout int) (TestSingleResponse, error) {
	client := &http.Client{Timeout: 30 * time.Second}
	reqBody, _ := json.Marshal(TestSingleRequest{Vless: vless, Timeout: timeout})

	resp, err := client.Post(c.baseURL+"/test-single", "application/json", bytes.NewReader(reqBody))
	if err != nil {
		return TestSingleResponse{}, fmt.Errorf("%w: %v", ErrDaemonUnreachable, err)
	}
	...
	if err := json.Unmarshal(body, &single); err != nil {
		return TestSingleResponse{}, fmt.Errorf("%w: %v", ErrDaemonParse, err)
	}
	return single, nil
}
